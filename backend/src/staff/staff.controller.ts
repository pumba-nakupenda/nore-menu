...
import { CreateStaffDto, LoginPosDto, UpdateStaffDto } from '../dto/staff.dto';
...
export class StaffController {
  ...
  @Post('login')
  async loginPOS(@Body() body: LoginPosDto) {
    return this.staffService.loginPOS(body.username, body.password);
  }

  @UseGuards(SupabaseGuard)
  @Post(':restaurantId')
  async createStaff(@Param('restaurantId') restaurantId: string, @Body() staffData: CreateStaffDto, @Request() req: any) {
  ...
  @UseGuards(SupabaseGuard)
  @Patch(':staffId')
  async updateStaff(@Param('staffId') staffId: string, @Body() staffData: UpdateStaffDto, @Request() req: any) {
    const token = req.headers.authorization?.split(' ')[1];
