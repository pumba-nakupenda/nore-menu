...
export default function ShopPage() {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-background"><Loader2 className="w-10 h-10 animate-spin text-brand" /></div>

    return (
        <div className="min-h-screen bg-background text-zinc-900 font-sans selection:bg-brand/10 selection:text-brand">
            <header className="fixed top-0 w-full z-50 border-b border-black/5 bg-background/80 backdrop-blur-md h-20">
                <div className="max-w-7xl mx-auto px-6 h-full flex justify-between items-center">
                    <Link href="/" className="flex items-center gap-2.5 group">
                        <ChevronLeft className="w-5 h-5 text-zinc-400 group-hover:-translate-x-1 transition-transform" />
                        <div className="bg-brand text-white p-2 rounded-xl shadow-lg"><ChefHat className="w-5 h-5" /></div>
                        <span className="text-xl font-serif font-bold tracking-tight text-brand">Nore Store</span>
                    </Link>
            ...
            <main className="pt-40 pb-32">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="max-w-3xl mb-20">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-emerald-50 text-brand rounded-full mb-6 border border-emerald-100">
                        ...
                        {items.length > 0 ? items.map((item, i) => (
                            <div key={item.id || i} className="group flex flex-col bg-white rounded-[3rem] border border-zinc-100 overflow-hidden hover:shadow-2xl transition-all duration-700">
                                <div className="aspect-square relative overflow-hidden bg-zinc-50 border-b border-zinc-50">
                                    ...
                                    <div className="absolute top-6 left-6 px-4 py-1.5 bg-white/90 backdrop-blur-md rounded-full text-[8px] font-black uppercase tracking-widest text-brand border border-black/5 shadow-sm">{item.tag || "Produit"}</div>
                                </div>
                                <div className="p-8 flex-1 flex flex-col">
                                    ...
                                    <div className="flex items-center justify-between pt-6 border-t border-zinc-50 mt-auto">
                                        <div className="flex flex-col">
                                            <span className="text-2xl font-black text-brand">{item.price?.toLocaleString() || 0}</span>
                                        ...
                                        <div className="flex items-center gap-3 bg-zinc-50 rounded-2xl p-1.5 border border-zinc-100">
                                            {cart[item.title] > 0 ? (
                                                <>
                                                    ...
                                                    <button onClick={() => updateQty(item.title, 1)} className="w-8 h-8 bg-brand text-white rounded-xl flex items-center justify-center shadow-lg"><Plus className="w-4 h-4" /></button>
                                                </>
                                            ) : (
                                                <button onClick={() => updateQty(item.title, 1)} className="px-6 py-2.5 bg-white text-brand text-[10px] font-black uppercase tracking-widest border border-zinc-200 rounded-xl hover:bg-zinc-900 hover:text-white transition-all shadow-sm">Acheter</button>
                                            )}
                                        </div>
                                    </div>
                                </div>
            ...
            {isCartOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-end">
                    ...
                    <div className="relative w-full max-w-md h-full bg-background shadow-2xl flex flex-col">
                            ...
                            {Object.entries(cart).map(([t, q]) => (
                                <div key={t} className="flex items-center justify-between group">
                                    ...
                                    <div className="flex items-center gap-4 bg-zinc-50 p-1 rounded-xl border border-zinc-100 ml-6">
                                        ...
                                        <button onClick={() => updateQty(t, 1)} className="w-8 h-8 bg-brand text-white rounded-lg flex items-center justify-center shadow-md"><Plus className="w-3 h-3" /></button>
                                    </div>
                                </div>
                            ))}
                        ...
                        <footer className="p-10 bg-zinc-50 border-t border-zinc-100 space-y-6">
                            <div className="flex justify-between items-end">
                                ...
                                <span className="text-3xl font-black text-brand">{cartTotal.toLocaleString()} FCFA</span>
                            </div>
                            <button disabled={cartCount === 0} onClick={sendOrder} className="w-full py-6 bg-brand text-white rounded-[2rem] font-black uppercase tracking-widest text-xs shadow-2xl flex items-center justify-center gap-4 hover:bg-brand-dark transition-all disabled:opacity-30">
